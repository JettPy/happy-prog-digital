...
def button_handlers(call):
    ...
    if call.data == 'lose_button':
        ...
        bot.send_message(call.message.chat.id, f"Конец игры, ваш счёт {score}. Давайте заново", reply_markup=inline_menu)
        cities_used.clear()
        score = 0
    elif call.data == 'score_button':
        # Если нажата кнопка "Сдаться". То выводим набранные очки и обнуляем счет
        bot.send_message(call.message.chat.id, f"У вас {score} очков", reply_markup=inline_menu)
        ...
        bot.send_message(call.message.chat.id, 'Выведено меню помощи', reply_markup=reply_menu)
